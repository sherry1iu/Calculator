...
blank_digits: process(y0,y1,y2,y3,anb)
begin
    an <= anb;
    if y3 = "0000" then
        if y2 = "0000" then
            if y1 = "0000" then
                an <= anb or "1110"; -- positive 1 digit
            else
                an <= anb or "1100"; -- positive 2 digits
            end if;
        else
            an <= anb or "1000"; -- positive 3 digits
        end if;  
    else
        if y2 = "0000" then
            if y1 = "0000" then
                if y0 = "0000" then
                    an <= anb or "1110"; -- zero (in case there is a negative sign)
                else
                    an <= anb or "0110"; -- negative 1 digit
                end if;
            else
                an <= anb or "0100"; -- negative 2 digits
            end if;
        else
            an <= anb; -- negative 3 digits (show the whole value)
        end if;      
    end if;
end process blank_digits;
...
with muxy select segh <=
	...
	"0111101" when x"d",	
	"1001111" when x"e",	
	"0000001" when x"f",	-- when y = 1111, negative sign  (f is not possible if bin2bcd is working correctly)
	"0000000" when others;	
